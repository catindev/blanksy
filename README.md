...
Открываете страницу и сразу сразу пишете заметку или статью. Нажимаете "Опубликовать" и получаете публичную ссылку и ссылку доступа для редактирования.
...
2. **Добавить кнопку** в `renderBlockToolbar()` (`src/views/toolbars.js`):
   ...
   <button
     type="button"
     class="bs_tool_button bs_block_btn"
     data-insert="mytype"
   >
     …
   </button>
