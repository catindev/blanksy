...
**1.4.0:**
- `POST /api/texts` — добавлен `optionalAuth`. Без авторизации поведение не изменилось.
- `PATCH /api/texts/:id` — принимает access token **или** SSO ownership.
- Новые: `POST /api/texts/:id/link`, `GET /api/my/texts`.

**1.3.0:** `diagram` nodes → HTTP 400.

## SSO / Identity contract

JWT payload принимаемый Bytext:
```json
{ "sub": "usr_abc123", "iss": "https://id.example.com", "aud": "bytext", "iat": 0, "exp": 0 }
```

- `sub` — непрозрачный userId, хранится как TEXT.
- `aud` — должен совпадать с `AUTH_JWT_AUDIENCE` (по умолчанию `"bytext"`).
...
**Различение токенов:**
- Bytext access token — base64url без точек.
- JWT — три части через точку (`xxx.yyy.zzz`).

## Expires policy

| Text type | expires_at |
|---|---|
...
`localStorage` keys:
- `bytext:access:{textId}` — access token
- `bytext:known_texts` — до 50 недавно открытых texts
...
- `text_owners` создаётся атомарно в той же транзакции что и text + access token.
