...
function validateMediaNode(node) {
  if (node.type === 'image') {
    ...
    if (!parsed) {
      throw new AppError(400, 'Image URL must be a public http/https URL with a supported extension');
    }
  }
...
function validateBodyLimits(body) {
  ...
  for (const node of body) {
    if (node.caption && node.caption.length > 300) {
      throw new AppError(400, 'Caption is too long');
    }

    switch (node.type) {
      case 'image':
        stats.images += 1;
        validateMediaNode(node);
        break;
      case 'video':
        stats.videos += 1;
        validateMediaNode(node);
  ...
  if (stats.images > 50) {
    throw new AppError(400, 'Too many images in blank body');
  }

  if (stats.videos > 20) {
    throw new AppError(400, 'Too many videos in blank body');
  }

  if (stats.links > 300) {
    throw new AppError(400, 'Too many links in blank body');
  }

  const jsonSize = Buffer.byteLength(JSON.stringify(body), 'utf8');
  if (jsonSize > 128 * 1024) {
    throw new AppError(400, 'Blank body exceeds 128 KB limit');
  }
}
...
function validateBlankInput(payload) {
  ...
  if (!parsed.success) {
    throw new AppError(400, 'Invalid blank payload', parsed.error.flatten());
  }
