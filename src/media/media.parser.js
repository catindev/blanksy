...
function parseImageUrl(input) {
  ...
  const hasAllowedExtension = IMAGE_EXTENSIONS.some((extension) => pathname.endsWith(extension));
...
function assertSafeExternalUrl(input) {
  return Boolean(parseHttpUrl(input));
}
