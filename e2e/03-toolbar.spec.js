...
const { setEditorContent } = require('./helpers');

test.describe('Text toolbar formatting', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/');
    await setEditorContent(page, { title: 'Тулбар-тест', body: 'Обычный текст для форматирования' });
  });

  // ── Bold ──────────────────────────────────────────────────────────────────

  test('selecting text shows the toolbar', async ({ page }) => {
    const para = page.locator('#bt_editor_root p').first();
    await para.selectText();
    await expect(page.locator('#bt_text_toolbar')).toBeVisible();
  });
  ...
  test('Bold button activates when bold text is selected', async ({ page }) => {
    ...
    await para.selectText();
    await expect(page.locator('[data-command="bold"]')).toHaveClass(/bt_tool_active/);
  });
  ...
  test('Italic button activates when italic text is selected', async ({ page }) => {
    ...
    await para.selectText();
    await expect(page.locator('[data-command="italic"]')).toHaveClass(/bt_tool_active/);
  });

  test('mixed state: partial bold shows bt_tool_mixed on bold button', async ({ page }) => {
    ...
    await expect(page.locator('[data-command="bold"]')).toHaveClass(/bt_tool_mixed/);
  });
  ...
  test('H2 button shows active when cursor is inside h2', async ({ page }) => {
    ...
    const h2 = page.locator('#bt_editor_root h2');
    await h2.selectText();
    await expect(page.locator('[data-command="heading"]')).toHaveClass(/bt_tool_active/);
  });
  ...
  test('H2 button toggles back to paragraph when clicked again', async ({ page }) => {
    ...
    await expect(page.locator('#bt_editor_root h2')).toHaveCount(0);
  });
  ...
  test('link button opens tooltip below selection', async ({ page }) => {
    ...
    await expect(page.locator('.bt_link_prompt')).toBeVisible();
  });
  ...
  test('entering a URL in the tooltip creates a link', async ({ page }) => {
    ...
    await page.locator('.bt_link_prompt input').fill('https://example.com');
  ...
  test('link button shows active when linked text is selected', async ({ page }) => {
    ...
    await expect(page.locator('[data-command="link"]')).toHaveClass(/bt_tool_active/);
  });
  ...
  test('clicking link button on linked text opens tooltip with existing href', async ({ page }) => {
    ...
    await para.locator('a').selectText();
    await page.locator('[data-command="link"]').click();
    const inputValue = await page.locator('.bt_link_prompt input').inputValue();
  ...
  test('clearing the URL in the tooltip removes the link', async ({ page }) => {
    ...
    await para.locator('a').selectText();
    await page.locator('[data-command="link"]').click();
    await page.locator('.bt_link_prompt input').clear();
    await page.keyboard.press('Enter');
...
test.describe('Quote and Enter behaviour', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/');
    await setEditorContent(page, { title: 'Цитата-тест', body: 'Текст первого абзаца' });
  });
  ...
  test('Quote button converts paragraph to blockquote', async ({ page }) => {
    ...
    await expect(page.locator('#bt_editor_root blockquote')).toBeVisible();
  });
  ...
  test('Enter inside blockquote creates a paragraph, not another blockquote (BUG-001)', async ({ page }) => {
    ...
    const bq = page.locator('#bt_editor_root blockquote');
    await bq.click();
...
test.describe('Empty paragraph accumulation', () => {
  test('repeated Enter in empty paragraph does not stack empty blocks', async ({ page }) => {
    await page.goto('/');
    await setEditorContent(page, { title: 'Enter-тест', body: 'Текст' });
  ...
  test('single empty paragraph between two content blocks is preserved', async ({ page }) => {
    await page.goto('/');
    await setEditorContent(page, { title: 'Отступ-тест', body: 'Первый абзац' });
